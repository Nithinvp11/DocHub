...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const updateData: {
      title?: string;
      path?: string;
      phase?: 'PLANNING' | 'DEVELOPMENT' | 'REVIEW' | 'COMPLETE' | 'ARCHIVED';
      type?:
        | 'GENERAL'
        | 'SPECIFICATION'
        | 'MEETING_NOTES'
    ...
    if (path === undefined && (title || phase || type)) {
      const nextTitle = title || document.title;
      const nextPhase = phase || document.phase;
      const nextType = type || document.type;
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Record a tombstone for documents that were synced to GitHub so manual export can delete remote files.
    const rawGitHubPath = document.githubPath || document.syncInfo?.githubPath || null;
    const normalizedGitHubPath = rawGitHubPath
      ? rawGitHubPath.replace(/^\/+/, '').replace(/^docs\//i, '')
      : null;
    const hasGitHubSyncEvidence = Boolean(
      document.githubSha ||
      document.syncInfo?.lastExternalHash ||
      document.syncInfo?.lastSyncedAt ||
      document.syncInfo?.lastCommitSha
    );

    if (normalizedGitHubPath && hasGitHubSyncEvidence) {
      const integration = await prisma.workspaceGitHubIntegration.findUnique({
        where: { workspaceId: document.workspaceId },
        select: { id: true },
      });
      if (integration) {
        const existingTombstone = await prisma.gitHubDeleteTombstone.findFirst({
          where: {
            workspaceId: document.workspaceId,
        ...
        if (!existingTombstone) {
          await prisma.gitHubDeleteTombstone.create({
            data: {
              workspaceId: document.workspaceId,
              githubPath: normalizedGitHubPath,
            },
